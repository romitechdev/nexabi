...
export default function TopCustomersPage() {
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4 sm:space-y-5 max-w-5xl">
        ...
        <div>
        <h1 className="text-xl sm:text-2xl font-bold text-white">Top Customers</h1>
        <p className="text-muted text-xs sm:text-sm mt-1">10 pelanggan dengan nilai transaksi tertinggi</p>
        </div>
      ...
      {!loading && customers.length >= 3 && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-2">
          {[1, 0, 2].map((idx) => {
            ...
            const rank = idx + 1;
            const medalColor = { 1: '#f59e0b', 2: '#cbd5e1', 3: '#cd7f32' }[rank];
            ...
            return (
              <div key={c.customer_id} className={`rounded-2xl border p-5 text-center ${heights[rank]}`}
                ...
                <p className="text-white font-semibold text-xs sm:text-sm truncate">{c.customer_name}</p>
        <p className="text-muted text-xs mt-0.5">{c.segment}</p>
        <p className="font-bold mt-2 text-xs sm:text-sm" style={{ color: medalColor }}>{fmt(c.monetary)}</p>
            ...
            <thead>
              <tr style={{ background: '#13161f', borderBottom: '1px solid #2a2d3a' }}>
                {['#', 'Customer', 'Segment', 'Recency', 'Frequency', 'Monetary', 'Proporsi', 'Cluster'].map(h => (
                  <th key={h} className="px-3 py-3 sm:px-4 sm:py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-muted whitespace-nowrap">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {loading ? [...Array(10)].map((_, i) => (
                <tr key={i} style={{ borderBottom: '1px solid #2a2d3a' }}>
                  {[...Array(8)].map((_, j) => <td key={j} className="px-3 py-3 sm:px-4 sm:py-3.5"><div className="h-4 animate-pulse rounded" style={{ background: '#2a2d3a' }} /></td>)}
                </tr>
              )) : customers.map((c, i) => {
                const pct = (c.monetary / maxMonetary) * 100;
                const badges = { 1: { bg: 'rgba(245,158,11,0.15)', color: '#fbbf24' }, 2: { bg: 'rgba(148,163,184,0.15)', color: '#cbd5e1' }, 3: { bg: 'rgba(205,127,50,0.15)', color: '#cd7f32' } };
                ...
                return (
                  <tr key={c.customer_id} className="hover:bg-white/[0.02] transition-colors" style={{ borderBottom: '1px solid #2a2d3a' }}>
                    <td className="px-3 py-3 sm:px-4 sm:py-3.5">
                      <span className="w-5 h-5 sm:w-6 sm:h-6 rounded-lg flex items-center justify-center text-xs font-bold"
                    ...
                    <td className="px-3 py-3 sm:px-4 sm:py-3.5 text-white font-semibold whitespace-nowrap">{fmt(c.monetary)}</td>
                    <td className="px-4 py-3.5" style={{ minWidth: '80px' }}>
