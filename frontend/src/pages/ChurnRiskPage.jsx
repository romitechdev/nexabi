...
export default function ChurnRiskPage() {
  ...
  return (
    <div className="p-6 space-y-5 max-w-6xl">
      <div>
        <h1 className="text-2xl font-bold text-white">Churn Risk Monitor</h1>
        <p className="text-muted text-sm mt-1">Pelanggan pasif yang sudah lama tidak bertransaksi — berisiko hilang</p>
      </div>

      {/* Summary cards */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="rounded-2xl border p-5" style={{ background: '#1a1d27', borderColor: '#2a2d3a' }}>
          <p className="text-muted text-sm mb-3">Total Berisiko Churn</p>
          {loading ? <div className="h-8 animate-pulse rounded" style={{ background: '#2a2d3a' }} /> : (
            <>
              <p className="text-3xl font-bold" style={{ color: riskLevel?.color }}>{data?.total_at_risk.toLocaleString('id-ID')}</p>
              <p className="text-muted text-xs mt-1">pelanggan perlu perhatian</p>
            </>
          )}
        </div>
        <div className="rounded-2xl border p-5" style={{ background: '#1a1d27', borderColor: '#2a2d3a' }}>
          <p className="text-muted text-sm mb-3">Avg Recency (Keseluruhan)</p>
          {loading ? <div className="h-8 animate-pulse rounded" style={{ background: '#2a2d3a' }} /> : (
            <>
              <p className="text-3xl font-bold text-white">{data?.avg_recency} <span className="text-lg font-normal text-muted">hari</span></p>
              <p className="text-muted text-xs mt-1">sejak transaksi terakhir rata-rata</p>
            </>
          )}
        </div>
        <div className="rounded-2xl border p-5" style={{ background: '#1a1d27', borderColor: '#2a2d3a' }}>
          <p className="text-muted text-sm mb-3">Level Risiko</p>
          {loading ? <div className="h-8 animate-pulse rounded" style={{ background: '#2a2d3a' }} /> : (
            <>
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl"
                ...
                <AlertTriangle className="w-4 h-4" style={{ color: riskLevel?.color }} />
                <span className="font-bold text-sm" style={{ color: riskLevel?.color }}>{riskLevel?.label}</span>
              </div>
      ...
      {/* At-risk table */}
      <div className="rounded-2xl border overflow-hidden" style={{ background: '#1a1d27', borderColor: '#2a2d3a' }}>
        <div className="px-5 py-4 border-b flex items-center justify-between" style={{ borderColor: '#2a2d3a', background: '#13161f' }}>
          <div className="flex items-center gap-2">
            <AlertTriangle className="w-4 h-4" style={{ color: '#f87171' }} />
            <h2 className="text-white font-semibold">Daftar Pelanggan Berisiko (Top 50)</h2>
          </div>
          <span className="text-xs px-2 py-1 rounded-lg" style={{ background: 'rgba(239,68,68,0.1)', color: '#f87171', border: '1px solid rgba(239,68,68,0.25)' }}>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ borderBottom: '1px solid #2a2d3a' }}>
                {['Nama', 'Segment', 'Recency (hari)', 'Frequency', 'Monetary', 'Risk Score'].map(h => (
            ...
            <tbody>
              {loading ? [...Array(6)].map((_, i) => (
                <tr key={i} style={{ borderBottom: '1px solid #2a2d3a' }}>
                  {[...Array(6)].map((_, j) => <td key={j} className="px-4 py-3"><div className="h-4 animate-pulse rounded" style={{ background: '#2a2d3a' }} /></td>)}
                </tr>
                ...
                return (
                  <tr key={c.customer_id} className="hover:bg-white/[0.02] transition-colors" style={{ borderBottom: '1px solid #2a2d3a' }}>
                    ...
                    <td className="px-4 py-3 text-muted">{c.segment}</td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-white">{c.frequency}x</td>
                    <td className="px-4 py-3 text-white">{fmt(c.monetary)}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <div className="h-1.5 w-16 rounded-full" style={{ background: '#2a2d3a' }}>
